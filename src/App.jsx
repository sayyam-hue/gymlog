...
export default function App() {
  ...
  const getMissedThisMonth = () => {
    ...
    for (let i = 1; i <= days; i++) {
      ...
      if (k >= APP_START && logs[k]?.activity === "missed") count++;
    }
    return count;
  };

  const getTotalSessions = () => Object.entries(logs).filter(([k, v]) => k >= APP_START && v.activity === "session").length;
  ...
  const monthSessionCount = () => {
    ...
    const days = getDaysInMonth(year, month);
    for (let i = 1; i <= days; i++) {
      const k = toKey(new Date(year, month, i));
      if (k >= APP_START && logs[k]?.activity === "session") c++;
    }
    return c;
  };
            ...
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
              ...
              <span style={{ fontSize: 11, color: "#aaa" }}>
                {streakInfo.type === "streak"
                  ...
                  : streakInfo.lastSession ? `Last session: ${fmtShort(streakInfo.lastSession)}` : "Starting 27 Mar 2026"}
              </span>
            </div>
        ...
        {tab === "calendar" && (
          <div style={s.body}>
            ...
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
              ...
              <span style={s.tag("r")}>{(() => { const { year, month } = calMonth; let c = 0; for (let i = 1; i <= getDaysInMonth(year, month); i++) { const k = toKey(new Date(year, month, i)); if (k >= APP_START && logs[k]?.activity === "missed") c++; } return c; })()} missed</span>
              <span style={s.tag("y")}>{(() => { const { year, month } = calMonth; let c = 0; for (let i = 1; i <= getDaysInMonth(year, month); i++) { const k = toKey(new Date(year, month, i)); if (k >= APP_START && logs[k]?.activity === "rest") c++; } return c; })()} rest</span>
            </div>
          ...
          <div style={s.overlay} onClick={e => { if (e.target === e.currentTarget) setShowModal(false); }}>
            <div style={s.modal}>
              ...
              <div style={s.mrow}>
                ...
                <input type="date" style={s.inp} value={modalDate} min={APP_START} max={toKey(today())} onChange={e => setModalDate(e.target.value)}/>
              </div>
              <div style={s.mrow}>
