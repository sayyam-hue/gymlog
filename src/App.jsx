...
export default function App() {
  ...
  const getStreakInfo = () => {
    ...
    if (logs[todayKey]?.activity === "session") {
      ...
      const streakStart = addDays(d, 1);
      return { type: "streak", count: streak, start: streakStart, end: new Date(t) };
    }
  ...
  const getTotalCoinsEarned = () => {
    return Object.values(earnedBadges).reduce((sum, b) => sum + (b.totalCoins || 0), 0);
  };
  ...
  const getDaysUntilRewards = () => {
    const unlock = parseKey(REWARDS_UNLOCK_DATE);
    const t = today();
    return Math.ceil((unlock - t) / (1000 * 60 * 60 * 24));
  };
  ...
  const checkMilestones = () => {
    ...
    const restTotal = getRestDaysTotal();
    const perfectWeeks = getPerfectWeeksCount();
    const consec4 = getConsecutive4PlusWeeks();
    const multiMuscle = getMultiMuscleSessionCount();
    ...
    // New peak — repeatable per best week count
    ...
    // Consistency
    // Show Up — 7 consecutive any activity
    let consec7 = 0; let d7 = addDays(today(), -1);
    for (let i = 0; i < 7; i++) { if (logs[toKey(d7)]?.activity) { consec7++; d7 = addDays(d7, -1); } else break; }
    if (consec7 >= 7) award("m25");

    // Smart Athlete — first rest day
    if (Object.entries(logs).some(([k, v]) => k >= APP_START && v.activity === "rest")) award("m26");

    // Never Miss a Day — 30 consecutive any activity
    let consec30 = 0; let d30 = addDays(today(), -1);
    for (let i = 0; i < 30; i++) { if (logs[toKey(d30)]?.activity && logs[toKey(d30)]?.activity !== "missed") { consec30++; d30 = addDays(d30, -1); } else break; }
    if (consec30 >= 30) award("m27");

    // Recovery Pro
    if (restTotal >= 10) award("m28");

    // Near Perfect Month — repeatable per month
    if (monthStats.missed <= 4) awardRepeatable("m29", `${todayMonth.year}-${todayMonth.month}`);

    // Perfect Month — repeatable per month
    if (monthStats.missed === 0) awardRepeatable("m30", `${todayMonth.year}-${todayMonth.month}`);

    // Bulletproof — 3 perfect months
    const perfectMonthCount = Object.keys(newBadges).filter(k => k.startsWith("m30_")).length;
    if (perfectMonthCount >= 3) award("m31");

    // The Comeback / Rise Again — check if today is session after miss streak
    ...
    // Consistent Beast — 3 months with 15+ sessions
    let months15 = 0;
    for (let i = 0; i < 12; i++) {
      const d = new Date(today().getFullYear(), today().getMonth() - i, 1);
      const ms = getMonthStats(d.getFullYear(), d.getMonth());
      if (ms.sessions >= 15) months15++;
    }
    if (months15 >= 3) award("m36");

    // Almost Perfect 90% — repeatable per month
    if (monthStats.attendance >= 90) awardRepeatable("m37", `${todayMonth.year}-${todayMonth.month}`);

    // Reliable — 80%+ attendance 3 consecutive months
    let consec80 = 0;
    for (let i = 0; i < 12; i++) {
      const d = new Date(today().getFullYear(), today().getMonth() - i, 1);
      const ms = getMonthStats(d.getFullYear(), d.getMonth());
      if (ms.attendance >= 80) consec80++; else break;
    }
    if (consec80 >= 3) award("m38");

    if (newPopups.length > 0) {
      saveBadges(newBadges);
      setPendingPopups(prev => [...prev, ...newPopups]);
    }
  };

  useEffect(() => {
    if (pendingPopups.length > 0 && !showPopup) {
      setShowPopup(pendingPopups[0]);
  ...
  const calDays = () => {
    ...
    for (let i = 1; i <= getDaysInMonth(year, month); i++) cells.push(i);
    return cells;
  };
  ...
  const getBadgeEarnedCount = () => {
    const uniqueIds = new Set(Object.keys(earnedBadges).map(k => k.split("_")[0]));
  ...
  const getBadgeProgress = (m) => {
    ...
    const muscleCount = getMuscleCount();
    const progressMap = {
      m1: [streak, 3], m2: [streak, 7], m3: [streak, 14], m4: [streak, 21],
      m5: [streak, 30], m6: [streak, 60], m7: [streak, 100],
      m8: [totalSess, 50], m9: [totalSess, 75], m10: [totalSess, 100],
      m11: [totalSess, 200], m12: [totalSess, 300], m13: [totalSess, 365],
      m14: [getPerfectWeeksCount(), 1], m15: [getPerfectWeeksCount(), 4],
      m16: [getPerfectWeeksCount(), 8], m17: [getConsecutive4PlusWeeks(), 4],
      m19: [Object.keys(muscleCount).length, 9], m20: [getMultiMuscleSessionCount(), 10],
      m21: [Math.max(...Object.values(muscleCount), 0), 25],
      m22: [Math.max(...Object.values(muscleCount), 0), 50],
      m23: [muscleCount["Legs"] || 0, 25], m24: [muscleCount["Core"] || 0, 20],
      m28: [getRestDaysTotal(), 10],
    };
    return progressMap[m.id] || null;
  };
  ...
  return (
    <div style={s.app}>
      <div style={s.phone}>
          ...
          <div style={s.body}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", gap:4 }}>
              ...
              {calDays().map((day, i) => {
                ...
                const beforeStart = k < APP_START;
                const a = beforeStart ? null : logs[k]?.activity;
                        ...
                        <div style={{ flex:1 }}>
                          ...
                          {progress && (
                            <div style={{ marginTop:4 }}>
                              <div style={{ height:4, background:"#f0f0f0", borderRadius:2, overflow:"hidden", width:"80%" }}>
                                <div style={{ height:"100%", background:"#1D9E75", borderRadius:2, width:`${Math.min(Math.round(progress[0]/progress[1]*100),100)}%` }}></div>
                              </div>
                  ...
                  {tierRewards.map(r => {
                    ...
                    return (
                      <div key={r.id} style={{ background:"#fff", border:"0.5px solid #e0e0e0", borderRadius:12, padding:"10px 12px", display:"flex", alignItems:"center", gap:10, marginBottom:8, opacity:redeemed?0.45:1 }}>
                        ...
                        {redeemed ? (
                          <div style={{ fontSize:11, color:"#aaa", background:"#f7f7f7", padding:"5px 10px", borderRadius:8 }}>Redeemed</div>
                        ) : !unlocked ? (
                          <div style={{ fontSize:11, color:"#BA7517", background:"#FAEEDA", padding:"5px 10px", borderRadius:8, whiteSpace:"nowrap" }}>Locked</div>
                        ) : canAfford ? (
                          <button onClick={() => setShowRedeemConfirm(r)} style={{ background:"#1D9E75", color:"#fff", border:"none", borderRadius:8, padding:"6px 12px", fontSize:12, fontWeight:500, cursor:"pointer", whiteSpace:"nowrap" }}>Redeem</button>
                        ) : (
        ...
        {/* WEIGHT TAB hidden in nav but accessible */}
